      ...
      if step % iter_size == 0:
        ...
        if step % bmuf_every == 0:
          if train_model.params['profile_steps'] and step % train_model.params['profile_steps'] == 0:
            fetches_vals = sess.run(fetches_blockend, feed_dict, options=run_options, run_metadata=run_metadata)
          else:
            fetches_vals = sess.run(fetches_blockend, feed_dict)
        else:
          if train_model.params['profile_steps'] and step % train_model.params['profile_steps'] == 0:
            fetches_vals = sess.run(fetches_inblock, feed_dict, options=run_options, run_metadata=run_metadata)
          else:
        ...
        def run_with_no_hooks(step_context, train_model, step, run_options, run_metadata):
          if train_model.params['profile_steps'] and step % train_model.params['profile_steps'] == 0:
